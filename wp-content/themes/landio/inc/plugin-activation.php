...
function uicore_ajax_plugins(){
    if ( ! check_ajax_referer( 'uicore_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
        wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'landio' ) ) );
    }
    $json = array();
    ...
    foreach ( $plugins as $plugin ) {
        if ( $_POST['slug'] == $plugin['slug'] ) {
            $json = array(
                ...
                'action2'       => - 1,
                'message'       => esc_html__( 'Installing...', 'landio' ),
            );
            break;
        }
    }
...
if (!function_exists('landio_plugins_activation')) :

    function landio_plugins_activation()
    {

        $plugins = uicore_get_plugins();

        $config = array(
            'id'           => 'landio',            // Unique ID for hashing notices for multiple instances of TGMPA.
            ...
            'strings'      => array(
                'page_title'                      => esc_attr__( 'Install Recommended Plugins', 'landio' ),
                'menu_title'                      => esc_attr__( 'Install Plugins', 'landio' )
            )
        );
...
add_action('tgmpa_register', 'landio_plugins_activation');
 ...
 */
function landio_get_plugin_activation_link( $plugin_base_name, $slug, $plugin_filename ) {
 ...
 */
function landio_get_plugin_install_link( $plugin_slug ){
 ...
 */
function landio_make_html_attributes( $attrs = array() ){
...
function landio_plugin_notice(){
    ...
    $plugins_title     = array(
        __('UiCore Framework', 'landio'),
        __('Elementor', 'landio'),
        __('Element Pack', 'landio')
    );
    ...
    foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
        ...
        $links_attrs[$key] = array(
            ...
            'data-activating-label' => sprintf( esc_attr__( 'Activating %s', 'landio' ), $progress_text_and_title ),
            'data-installing-label' => sprintf( esc_attr__( 'Installing %s', 'landio' ), $progress_text_and_title ),
            'data-activate-label'   => sprintf( esc_attr__( 'Activate %s'  , 'landio' ), $progress_text_and_title ),
            'data-install-label'    => sprintf( esc_attr__( 'Install %s'   , 'landio' ), $progress_text_and_title ),

            'data-activate-url'     => landio_get_plugin_activation_link( $plugins_base_name[$value], $plugins_slug[$value], $plugins_filename[$value] ),
            'data-install-url'      => landio_get_plugin_install_link( $plugins_slug[$value] ),
        ...
        if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Install %s', 'landio' ), $progress_text_and_title );
            ...
            $links_attrs[$key]['button_label'] = sprintf( esc_html__( 'Activate %s', 'landio' ), $progress_text_and_title );
        }
    }
?>
<script>
                            ...
                            if ( response.url ) {
                                globalAjax(
                                    ...
                                    function(){ // success callback
                                        buttonStatusInstalled( 'All plugins intalled and activated sucessfully!' );
                                        activatePlugin($data);
                                    },
...
.uicore-close:hover::before{
    ...
    <div class="uicore-msg" style="background-image:url(<?php echo get_template_directory_uri(); ?>/assets/img/demo-import-bg.jpg)">
        <h3 class="uicore-title-default"><?php printf( __( 'Thanks for choosing %s', 'landio' ), UICORE_THEME_NAME ); ?></h3>
        <p class="uicore-description-default"><?php echo __( 'Please install and activate the required plugins to gain access to all the theme functionalities and features.', 'landio' ); ?></p>
        <p class="submit">
            <a class="uicore-btn-default" data-info='<?php echo wp_json_encode( $links_attrs);?>' >Install Required Plugins</a>
            <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'uicore-fw-notice', 'install' ), 'uicore-fw-notice', '_notice_nonce' ) ); ?>" class="notice-dismiss uicore-close"><span class="screen-reader-text"><?php _e( 'Skip', 'landio' ); ?></span></a>
        </p>
    </div>
<?php
}
add_action( 'admin_notices', 'landio_plugin_notice' );
...
function uicore_hide_notice() {
    ...
    if ( isset( $_GET['uicore-fw-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_notice_nonce'], 'uicore-fw-notice' ) ) {
            wp_die( __( 'Authorization failed. Please refresh the page and try again.', 'landio' ) );
        }
